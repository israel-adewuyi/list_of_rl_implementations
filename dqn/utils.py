...
def linear_schedule(
    current_step: int, start_eps: float, end_eps: float, exploration_fraction:
    float, total_timesteps: int
) -> float:
    return start_eps + (end_eps - start_eps) * min(current_step / (exploration_fraction * total_timesteps), 1)
